...
const Kids = () => {
    ...
    const shoesCollection = collection(db, "KidsShoes")
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                ...
                console.error("Failed to fetch KidsShoes data", error)
            }
        }
        getData()
    }, [])
    ...
    const handleDelete = (shoeId) => {
        ...
        else return (<Col><Button variant="danger" onClick={() => deleting(shoeId)} style={{ marginLeft: '5px' }}>Delete</Button></Col>)
    }
    ...
    const deleting = async (id) => {
        try {
            const docToDelete = doc(db, "KidsShoes", id)
